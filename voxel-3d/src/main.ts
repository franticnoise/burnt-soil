...
class VoxelTerrain {
  ...
  constructor(width: number, depth: number, voxelSize: number) {
    this.width = width;
    this.depth = depth;
...
class Tank {
  ...
  setAimPitch(angle: number) {
    ...
    const clampedAngle = Math.max(0, angle);
    this.loadedTurretPivot.rotation.x = -clampedAngle;
    this.loadedTurretPivot.rotation.z = 0;
  }
  ...
  private setupLoadedTurretRig() {
    ...
    this.loadedModel.traverse((node) => {
      ...
      meshInfos.push({ mesh: node, center, size, score });
    });
    ...
    const pivotPos = new THREE.Vector3(
      THREE.MathUtils.lerp(modelCenter.x, barrelRef.center.x, 0.45),
      THREE.MathUtils.lerp(modelCenter.y, barrelRef.center.y, 0.7),
      THREE.MathUtils.lerp(modelCenter.z, barrelRef.center.z, 0.35),
    );
    ...
    let barrelMembers = meshInfos.filter((info) => {
    ...
    if (barrelMembers.length === 0) {
      barrelMembers = [barrelRef];
    }

    for (const member of barrelMembers) {
      pivot.attach(member.mesh);
    }
...
class BurntSoil3D {
  ...
  private readonly terrain = new VoxelTerrain(WORLD_SIZE, WORLD_SIZE, VOXEL_SIZE);
  ...
  private async loadExternalModels() {
    const [playerModel, enemyModel, bulletModel] = await Promise.all([
      this.loadT90Model(),
      this.loadPanzerModel(),
      this.loadBulletModel(),
    ]);

    if (playerModel) {
      // Clone BEFORE setModel modifies the hierarchy (setupLoadedTurretRig reparents meshes)
      const playerClone = this.tanks[2] ? playerModel.clone(true) : null;
      this.tanks[0].setModel(playerModel, -0.12, new THREE.Vector3(-Math.PI * 0.5, 0, 0));
      if (this.tanks[2] && playerClone) {
        this.tanks[2].setModel(playerClone, -0.12, new THREE.Vector3(-Math.PI * 0.5, 0, 0));
      }
    }
    if (enemyModel) {
      const enemyClone = this.tanks[3] ? enemyModel.clone(true) : null;
      this.tanks[1].setModel(enemyModel, 0.25, new THREE.Vector3(Math.PI * 0.5, Math.PI, Math.PI * 0.5));
      if (this.tanks[3] && enemyClone) {
        this.tanks[3].setModel(enemyClone, 0.25, new THREE.Vector3(Math.PI * 0.5, Math.PI, Math.PI * 0.5));
      }
    }
    if (bulletModel) {
  ...
  private async loadT90Model(): Promise<THREE.Object3D | null> {
    try {
  ...
  private async loadPanzerModel(): Promise<THREE.Object3D | null> {
    try {
  ...
  private async loadBulletModel(): Promise<THREE.Object3D | null> {
    try {
      ...
      this.normalizeModel(model, 0.25);
      return model;
    } catch {
      return null;
    }
  ...
  private updateHud() {
    ...
    const turn = tank.team === "player" ? "PLAYER" : "AI";
    const status = this.gameOver
      ? this.winner === "player"
        ? "🏆 VICTORY!"
        : "💀 DEFEAT!"
      : `Turn: <span style="color:${tank.team === "player" ? "#ea6153" : "#4ea9de"}">${turn}</span>`;

    const windDir = this.wind > 0.3 ? "→" : this.wind < -0.3 ? "←" : "·";
    const windColor = Math.abs(this.wind) > 3 ? "#ff9944" : "#aaa";

    this.hud.innerHTML = [
      `<b style="font-size:15px;color:#ffd666">BURNT SOIL 3D</b>`,
      status,
      `<span style="color:${windColor}">Wind: ${windDir} ${this.wind >= 0 ? "+" : ""}${this.wind.toFixed(2)}</span>`,
      `Fuel: <span style="color:${this.turnFuel < 25 ? "#ff6644" : "#aaa"}">${this.turnFuel.toFixed(0)}</span>`,
      ...
      `<span style="font-size:11px;color:#886">Wheel zoom | LMB orbit | MMB reset</span>`,
    ].join("<br>");
  ...
  private fireCurrentTank() {
    ...
    const shotVelocity = this.buildShotVelocity(tank);
  ...
  private buildShotVelocity(tank: Tank): THREE.Vector3 {
    const horizontalMag = tank.power * 0.35;
  ...
  private explodeAt(position: THREE.Vector3, power: number) {
    const craterRadius = THREE.MathUtils.clamp(1.2 + power * 0.045, 1.3, 4.4);
    ...
    for (const tank of this.tanks) {
      ...
      if (distance < 5.2) {
        ...
        const damage = Math.max(0, proximity) * 90;
        tank.health -= damage;
  ...
  private updateProjectile(dt: number) {
    ...
    if (worldPos.y <= terrainY + 0.2) {
      this.explodeAt(worldPos, p.owner.power);
      this.scene.remove(p.mesh);
      this.currentProjectile = null;
      this.advanceTurn();
    }
  ...
  private moveTankForward(tank: Tank, direction: number, dt: number) {
    if (this.turnFuel <= 0 || !tank.alive) {
      return;
    }

    const maxDistanceByFuel = this.turnFuel / FUEL_BURN_PER_UNIT;
    ...
    this.turnFuel = Math.max(0, this.turnFuel - moved * FUEL_BURN_PER_UNIT);
  ...
  private updatePlayerInput(dt: number) {
    const tank = this.tanks[this.currentTankIndex];
    if (this.gameOver || tank.team !== "player" || this.shotInFlight || !tank.alive) return;
  ...
  private readonly onKeyDown = (event: KeyboardEvent) => {
    ...
    if (event.code === "Space") {
      ...
      if (!this.gameOver && this.tanks[this.currentTankIndex]?.team === "player") {
        this.fireCurrentTank();
      }
    }
  };
...
const game = new BurntSoil3D(canvas);
